...
#
#
# $Id: 73_AMADCommBridge.pm 20129 2019-09-08 08:08:31Z CoolTux $
#
###############################################################################
##
##
...
sub ResponseProcessing($$) {
    ...
    Log3( $bname, 4,
        "AMADCommBridge ($name) - Receive RAW Message in Debugging Mode: $json"
    );
    ...
    if ($@) {
        Log3( $bname, 4,
            "AMADCommBridge ($name) - JSON error while request: $@" );
...
"header lines: \r\n AMADCommBridge receive a JSON error\r\n AMADCommBridge to do nothing\r\n";
    ...
    if ( defined( $decode_json->{firstrun} ) and ( $decode_json->{firstrun} ) )
    {
        ...
        $fhemDevice = $decode_json->{firstrun}{fhemdevice}
          if ( defined( $decode_json->{firstrun}{fhemdevice} ) );
    ...
    if ( !defined($amad_id) or !defined($fhemDevice) ) {
        readingsSingleUpdate( $bhash, 'transmitterERROR',
            $hash->{NAME} . ' has no correct amad_id', 1 );
        Log3( $bname, 4,
"AMADCommBridge ($name) - ERROR - no device name given. please check your global variable amad_id in automagic"
        );
...
"header lines: \r\n AMADCommBridge receive no device name. please check your global variable amad_id in automagic\r\n FHEM to do nothing\r\n";
    ...
    if ( defined($fhemcmd) and ($fhemcmd) ) {
        if ( $fhemcmd eq 'setreading' ) {
            return Log3( $bname, 3,
"AMADCommBridge ($name) - AMADCommBridge: processing receive no reading values from Device: $fhemDevice"
              )
              unless (
                (
                    defined( $decode_json->{payload} )
                    and ( $decode_json->{payload} )
                )
                or ( defined( $decode_json->{firstrun} )
                    and ( $decode_json->{firstrun} ) )
              );

            Log3( $bname, 4,
"AMADCommBridge ($bname) - AMADCommBridge: processing receive reading values - Device: $fhemDevice Data: $decode_json->{payload}"
              )
              if ( defined( $decode_json->{payload} )
                and ( $decode_json->{payload} ) );
...
"header lines: \r\n AMADCommBridge receive Data complete\r\n FHEM was processes\r\n";
            ...
            AnalyzeCommandChain( $bhash, 'set ' . $fhemCmd )
              if ( AttrVal( $bname, 'fhemControlMode', 'trigger' ) eq
                'setControl' );
            readingsSingleUpdate( $bhash, 'receiveFhemCommand',
                'set ' . $fhemCmd, 1 )
              if (
                AttrVal( $bname, 'fhemControlMode', 'trigger' ) eq 'trigger' );
            Log3( $bname, 4,
"AMADCommBridge ($name) - CommBridge: set reading receive fhem command"
            );
...
"header lines: \r\n AMADCommBridge receive Data complete\r\n FHEM response\r\n";
            ...
            my $fhemCmd =
              lc( encode_utf8( $decode_json->{payload}{voiceinputdata} ) );
            ...
            readingsEndUpdate( $bhash, 1 );
            Log3( $bname, 4,
"AMADCommBridge ($name) - CommBridge: set reading receive voice command: $fhemCmd from Device $fhemDevice"
            );
...
"header lines: \r\n AMADCommBridge receive Data complete\r\n FHEM was processes\r\n";
            ...
            $response =
              ReadingsVal( $datavalue[0], $datavalue[1], $datavalue[2] );
            $c = $hash->{CD};
            print $c "HTTP/1.1 200 OK\r\n",
              "Content-Type: text/plain\r\n",
              "Connection: close\r\n",
            ...
            Log3( $bname, 4,
"AMADCommBridge ($name) - CommBridge: receive fhem-function command"
            );

            if ( AttrVal( $bname, 'enableSubCalls', 0 ) == 1 ) {

                $response = AnalyzePerlCommand( $bhash, '{' . $fhemCmd . '}' )
                  ; # AnalyzePerlCommand is new https://forum.fhem.de/index.php/topic,89619.msg820964.html#msg820964
...
"header lines: \r\n Attribut enableSubCalls is not set or value is 0\r\n FHEM to do nothing\r\n";
                Log3 $bname, 3,
"AMADCommBridge ($name) - Attribut enableSubCalls is not set or value is 0, FHEM to do nothing";
            }
    ...
    if ($buffer) {
        foreach my $c ( split //, $buffer ) {
            if ( $open == $close && $open > 0 ) {
                $tail .= $c;
                Log3( $name, 5,
                    "AMADCommBridge ($name) - $open == $close && $open > 0" );

            }
            elsif ( ( $open == $close ) && ( $c ne '{' ) ) {

                Log3( $name, 5,
"AMADCommBridge ($name) - Garbage character before message: "
                      . $c );
    ...
    Log3( $name, 5,
        "AMADCommBridge ($name) - return msg: $msg and tail: $tail" );
    return ( $msg, $tail );
}
...
=for :application/json;q=META.json 73_AMADCommBridge.pm
{
